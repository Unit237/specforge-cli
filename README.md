...
| `spec watch` | Long-running daemon. Broadcasts your prompts + dirty files and tails the workspace stream: all of your own conversations plus every conversation authored by an accepted teammate. Every row names its chat; Codex rows use textual `USER`, `UPDATE`, and `ANSWER` badges so prompts, progress commentary, and conclusions remain distinct without relying on color. Internal reasoning and completion sentinels stay hidden. `--mirror` also writes incoming peer events to `prompts/captured/peers/<handle>/`. |
